<aside class="w-64 h-screen bg-gradient-to-b from-red-900 to-blue-900 border-r border-red-700/50 flex flex-col fixed top-0 left-0 z-30 shadow-2xl">
    ...
    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        ...
        <div class="pt-6">
            ...
            <!-- Content Management -->
            <a href="{{ route('admin.contents.index') }}" class="group flex items-center px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.contents.index') ? 'bg-gradient-to-r from-yellow-600 to-yellow-500 text-white shadow-lg shadow-yellow-500/25' : 'text-yellow-300 hover:bg-yellow-700/50 hover:text-white' }}">
                <div class="flex items-center justify-center w-8 h-8 mr-3 rounded-lg {{ request()->routeIs('admin.contents.index') ? 'bg-white/20' : 'bg-yellow-700/50 group-hover:bg-yellow-600/50' }}">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            ...
            <a href="{{ route('admin.contents.reported') }}" class="group flex items-center px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.contents.reported') ? 'bg-gradient-to-r from-pink-600 to-pink-500 text-white shadow-lg shadow-pink-500/25' : 'text-pink-300 hover:bg-pink-700/50 hover:text-white' }}">
                <div class="flex items-center justify-center w-8 h-8 mr-3 rounded-lg {{ request()->routeIs('admin.contents.reported') ? 'bg-white/20' : 'bg-pink-700/50 group-hover:bg-pink-600/50' }}">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
